...
// Find the map start
let mapStart = content.indexOf('{PAYMENT_METHODS.map(method => {');
if (mapStart !== -1) {
  let mapEnd = content.indexOf('})}', mapStart);
  
  let newMapStr = `                    {PAYMENT_METHODS.map(method => {
                    ...
                    `;
  content = content.substring(0, mapStart) + newMapStr + content.substring(mapEnd);
